...
def remote_install(
    ...
    with Progress(
        ...
        with ssh.SSHSession() as ssh_session:
            _install_miniconda(progress, view, ssh_session)
        
            view.update(20, "Setting up environment...")
            
            _setup_environment(progress, ssh_session)
                
            view.update(40, "Installing extensions...")

            _install_extensions(progress, view, ssh_session)
            
            view.update(95, "Uploading remote script...")
            
            ssh_session.execute_command("mkdir -p ~/LLMPipe")
            ssh_session.execute_command("mkdir -p ~/LLMPipe/results")
        
            ssh_session.upload_file(remote_script_path, "LLMPipe/llm_pipe.py")
            
            view.update(100, "Installed.")

def _install_miniconda(progress: Progress, view: display.InstallProgressView, ssh_session: ssh.SSHSession):
    try:
        ssh_session.execute_command('test -d "~/miniconda3"')
...
def _setup_environment(progress: Progress, ssh_session: ssh.SSHSession):
    remove_command = ""
    try:
        ssh_session.execute_command(f'test -d "~/miniconda3/envs/plantcad_env"')
...
def _install_extensions(progress: Progress, view: display.InstallProgressView, ssh_session: ssh.SSHSession):
