...
def get_files(
    ...
    try:
        with ssh.SSHSession() as ssh_session:
            ...
            ssh_session.download_dir(f"LLMPipe/results/{session_name}/", f"{download_path}/{session_name}_results/")
...
def start_session(
    ...
    try:
        with ssh.SSHSession() as ssh_session:
            ...
            return_code = session_manager.start_session(session_name, fasta_file, language_model, context_window, align, ssh_session)
    except ssh.ConnectionError:
    ...
    match return_code:
        case 1:
            ...
            raise typer.Exit(3)
        case 2:
            display.show_error_message("Fasta file failed to transfer.")
            raise typer.Exit(4)
        case 3:
...
def view_sessions():
    ...
    if json_manager.load_login_info():
        logged_in = True
    else:
        logged_in = False
    ...
    try:
        ...
        with ssh.SSHSession() as ssh_session:
            ...
            login_info = json_manager.load_login_info()
            synced_sessions = session_manager.get_synced_sessions(ssh_session)
            ...
            _update_running_sessions(running_sessions=running_sessions, login_info=login_info, ssh_session=ssh_session)
...
def delete_session(
    ...
    try:
        with ssh.SSHSession() as ssh_session:
            ...
            session_status = ssh_session.check_session_in_progress(session_name)
            ...
            session_manager.delete_session(session_name, session_status, login_info["hostname"], login_info["username"], ssh_session)
    ...
    try:
        with ssh.SSHSession() as ssh_session: 
            ...
            for session in sessions:
                try:
                    session_status = ssh_session.check_session_in_progress(session["name"])
                    session_manager.delete_session(session["name"], session_status, login_info["hostname"], login_info["username"], ssh_session)
...
def _sync_sessions(ssh_session: ssh.SSHSession):
    """Sync the locally logged sessions with the remote files."""
    login_info = json_manager.load_login_info()
    
    # Sync finished remote sessions
    finished_sessions = ssh_session.get_finished_remote_sessions()
    ...
    running_sessions = ssh_session.get_running_remote_sessions()
    ...
    logged_sessions = session_manager.get_synced_sessions(ssh_session)
...
def _update_running_sessions(
    ...
    readers = [
        ssh.StreamReader(
            ssh_session.stream_command(f"tail -n 1 -f ~/LLMPipe/{session_info["name"]}.log")
        )   
        for session_info in running_sessions
    ]
